...
const LEXICON_NSIDS = [
	'defs', // app-private defs (still used by shelf/bookShelving/actor records)
	'shelf',
	'bookShelving',
	'actor',
];
...
describe('pages', () => {
	it('serves the home page at /', async () => {
		...
		expect(body).toContain('community.lexicon.book');
		expect(body).toContain('did-ssr');
	});

	it('serves the queries page', async () => {
		const res = await app.request('/queries');
		expect(res.status).toBe(200);
		expect(res.headers.get('content-type')).toContain('text/html');
		const body = await res.text();
		expect(body).toContain('community.lexicon.book.searchEditions');
		expect(body).toContain('did-ssr');
	});

	it('serves the procedures page', async () => {
		const res = await app.request('/procedures');
		expect(res.status).toBe(200);
		expect(res.headers.get('content-type')).toContain('text/html');
		const body = await res.text();
		expect(body).toContain('did-ssr');
		expect(body).not.toContain('community.lexicon.book.searchEditions');
	});
...
describe('static lexicon serving', () => {
	...
	it('serves community.lexicon.book.edition.json', async () => {
		const res = await app.request('/lexicons/community/lexicon/book/edition.json');
		expect(res.status).toBe(200);
		const body = await res.json();
		expect(body.id).toBe('community.lexicon.book.edition');
	});
});
