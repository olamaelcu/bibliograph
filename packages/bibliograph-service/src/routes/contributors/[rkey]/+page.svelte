<script lang="ts">
  ...
  let { data }: { data: { kind: 'contributors'; rkey: string; notFound: boolean; value?: DetailValue } } = $props();
</script>
...
<main>
  ...
  {#if data.notFound}
    <section class="missing">
      ...
      <p class="muted">No contributor record exists for rkey <code>{data.rkey}</code>.</p>
    </section>
  {:else if data.value}
