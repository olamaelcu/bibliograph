<script lang="ts">
  ...
  let { data }: { data: { kind: 'editions'; rkey: string; notFound: boolean; value?: DetailValue } } = $props();
</script>
...
<main>
  ...
  {#if data.notFound}
    <section class="missing">
      ...
      <p class="muted">
        No edition record exists for rkey <code>{data.rkey}</code> at <code>{data.kind}</code>.
      </p>
    </section>
  {:else if data.value}
